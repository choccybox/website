...
userinfo.get('/userinfo', async (req, res) => {
  try {
    ...
    const userInfo = {
      ...
      avatar: userResponse.data.avatar ? `https://cdn.discordapp.com/avatars/${userResponse.data.id}/${userResponse.data.avatar.startsWith('a_') ? `${userResponse.data.avatar}.gif?size=280` : `${userResponse.data.avatar}.webp?size=300`}` : null,
